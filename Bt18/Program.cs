...
namespace BaiTap
{
    ...
    public class Bt18
    {
        public static void Main(String[] args)
        {
            ...
            var listResult = students.GroupBy(s => s.Score>=8 ? "Gioi" : s.Score >=6 ? "Kha" : "Trung binh");
            foreach (var i in listResult) 
            {
                Console.WriteLine($"Danh sach sinh vien {i.Key} la:  ");
                foreach(var sv in i)
                {
                    Console.WriteLine($"- {sv.Name} (Điem: {sv.Score})");
                }
            }
            Console.ReadKey();
